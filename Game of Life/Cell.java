...
public class Cell {
    ...
    public int getNeighborsWrap(Cell[][] cells) {
        calcNeighborsWrap(cells);
    ...
    public void calcNeighborsWrap(Cell[][] cells) {
        myNeighbors = 0;
        ...
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
                if (!(i == 1 && j == 1)) {
                    ...
                    if (x >= 0 && x < 100 && y >= 0 && y < 80 && !(i == 1 && j == 1)) {
                        myNeighbors += (cells[y][x].getAlive() ? 1 : 0);
                    }
                }
            }
        }
